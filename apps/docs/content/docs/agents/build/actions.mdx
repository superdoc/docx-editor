...
Actions that accept `changeMode` record the edit as a tracked change when it is `"tracked"` and edit directly otherwise. The tool schema advertises the argument once for the whole tool, so the table below is the place that says which actions honor it. Actions marked **No** do not accept `changeMode` because they manage comments, review decisions, or local undo history rather than creating tracked document edits.
...
| `move_range`        | `fromText`, `toText?`, `afterText` or `beforeText`                | Yes     | Moves a block range or a visual section identified by text. Ranges containing tables, lists, or images are refused |
...
| `split_list`       | `anchorText`, `restartNumbering?`                                                                    | Yes     | Splits one list into two at an item                                                    |
...
| `apply_style`              | `selector`, then one of `styleId`, `headingLevel`, or `likeText`                                                                                 | Yes     | `likeText` copies another block's style and effective look     |
| `format_paragraph`         | `selector`, `alignment`                                                                                                                          | Yes     | Tracked mode records the former alignment as a property change |
| `set_paragraph_spacing`    | `selector`, `lineSpacing?`, `spaceBefore?`, `spaceAfter?`                                                                                        | Yes     |                                                                |
...
| Action              | Arguments                 | Tracked | Notes                                                                    |
| ------------------- | ------------------------- | ------- | ------------------------------------------------------------------------ |
| `insert_page_break` | `selector`                | Yes     | Sets a page break before the block instead of inserting empty paragraphs |
| `add_hyperlink`     | `text`, `url`, `tooltip?` | Yes     | Turns existing text into a link                                          |
...
| Action                | Arguments                                                                   | Tracked | Notes                                              |
| --------------------- | --------------------------------------------------------------------------- | ------- | -------------------------------------------------- |
| `style_table`         | `tableOrdinal?`, `accentColor?`                                             | Yes     | Accent header, bold first column, banded rows      |
| `move_table`          | `tableOrdinal?`, `placement`                                                | Yes     | Moves the whole table with its content in one call |
| `delete_table`        | `tableOrdinal?`                                                             | Yes     |                                                    |
| `insert_table_row`    | `tableOrdinal?`, `rowIndex?`, `position?`, `cellTexts?`                     | Yes     |                                                    |
