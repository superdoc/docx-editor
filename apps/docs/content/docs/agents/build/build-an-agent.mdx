...
A successful receipt is not proof of a change either. An action whose target matched nothing, or one that resolved to a no-op, still reports `ok`, so the example counts a mutation only when the receipt's before and after revisions differ.
...
[Tools](/agents/build/tools) explains the action surface, how to narrow it, and how to replay a tool call deterministically without the model. [Reuse one client across many operations](/agents/automation/node-sdk#reuse-one-client-across-many-operations) covers where the client and the open document live relative to the loop: never inside it. [Safety](/agents/operate/safety) covers what to settle before an agent touches documents that matter.
