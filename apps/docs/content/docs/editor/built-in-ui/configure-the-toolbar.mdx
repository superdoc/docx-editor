...
## Add the Watermark dialog

Enable the optional Watermark button with `ui.toolbar.includeItems: ['watermark']`. It opens the text and picture
watermark dialog and uses the toolbar's existing overflow menu in narrow containers. See
[Configure document watermarks](/editor/built-in-ui/watermarks) for runnable Vanilla and React examples.
