...
## Load the optional substitute pack

Install the font pack to supply known substitutes such as Carlito for Calibri, Caladea for Cambria, and Liberation
Serif for Times New Roman:

```sh
npm install @superdoc/fonts
```

Add the pack to your existing Editor configuration. Your bundler serves the package's WOFF2 files; SuperDoc loads the
faces needed by the document.
...
<FrameworkExampleTabs>
  ...
  <FrameworkExample filename='src/App.tsx' framework='React'>
    ...
    export default function App() {
      return <SuperDocEditor document='/sample.docx' fonts={superdocFonts} />;
    }
    ```
...
The pack contains substitutes, not proprietary Microsoft font files, and does not cover every family. It does not
guarantee identical Word rendering. The Quickstart's Aptos families still need an appropriate provider; continue below
to host licensed files or configure a mapping. Check font reports and compare line and page breaks for your documents.

To load only selected substitutes, import `createSuperDocFonts` from the same package and use logical Word family names:
`createSuperDocFonts({ include: ['Calibri', 'Cambria'] })`.

For a plain-script integration, load the version-pinned `dist/superdoc-fonts.min.js` browser build and pass
`SuperDocFonts.superdocFonts`. When self-hosting, keep the package's `assets` directory beside `dist` so its relative
font URLs resolve. Use one loading path per page.

If you use `@superdoc-dev/fonts`, uninstall it, install `@superdoc/fonts`, and update imports and any CDN paths to the
new name. The `superdocFonts`, `createSuperDocFonts`, and `resolveBundledFontAssetUrl` exports keep the same usage.
New stable releases retain the old name as a deprecated compatibility mirror. Avoid loading both packs.
...
Registering a family supplies its files; it does not add an unused family to the font picker. To let users apply a new
family, add `{ value: 'Brand Sans', label: 'Brand Sans', previewFamily: 'Brand Sans' }` to `ui.toolbar.fontOptions` and
keep the matching `fonts.families` entry. This configured list owns the dropdown, so include every choice you want to
offer. See [Configure the built-in toolbar](/editor/built-in-ui/configure-the-toolbar).
