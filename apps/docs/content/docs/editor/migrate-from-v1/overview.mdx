...
V1 applications supplied an external Y.Doc and provider through `modules.collaboration`. V2 instead takes a supported provider target and owns the provider lifecycle. Do not port the external pair directly. [Use your own provider](/editor/collaboration/use-your-own-provider) shows how to move a Hocuspocus setup and when to write a provider extension.
